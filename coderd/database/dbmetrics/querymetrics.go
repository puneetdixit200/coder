...
func (m queryMetricsStore) InsertChatWithSnapshot(ctx context.Context, arg database.InsertChatWithSnapshotParams) (database.Chat, error) {
	start := time.Now()
	r0, r1 := m.s.InsertChatWithSnapshot(ctx, arg)
	m.queryLatencies.WithLabelValues("InsertChatWithSnapshot").Observe(time.Since(start).Seconds())
	m.queryCounts.WithLabelValues(httpmw.ExtractHTTPRoute(ctx), httpmw.ExtractHTTPMethod(ctx), "InsertChatWithSnapshot").Inc()
	return r0, r1
}
