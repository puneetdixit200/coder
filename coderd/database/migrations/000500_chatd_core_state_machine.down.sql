...
-- 2. Drop the queue version triggers and function.
...
-- 3. Drop the message revision triggers and functions.
...
-- 4. Drop chat_heartbeats (and its index by association).
DROP TABLE IF EXISTS chat_heartbeats;

-- 5. Drop the queue-order index.
DROP INDEX IF EXISTS idx_chat_queued_messages_chat_position_id;

-- 6. Drop chat_queued_messages.position and its default sequence, plus
-- created_by.
...
-- 7. Drop chat_messages.revision.
ALTER TABLE chat_messages
    DROP COLUMN IF EXISTS revision;

-- 8. Drop the new chats columns.
ALTER TABLE chats
...
-- 9. Recreate chats_expanded with the pre-migration field list.
...
-- 10. The `interrupting` chat_status enum value is intentionally left
