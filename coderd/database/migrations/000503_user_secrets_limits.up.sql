...
--
--   * count_limit = 50: backstop, well under the ~120-secret ceiling
--     where the manifest would overflow at 32 KiB per value
--     (MaxSecretValueSize).
--
...
--
-- octet_length(value) measures stored bytes. In encrypted deployments
-- stored bytes exceed plaintext (AES-GCM + base64 ~1.33x), which is
-- fine: the limits we defend measure transmitted bytes and stored
-- bytes upper-bound those.
--
-- Keep the literals below in sync with codersdk.MaxUserSecrets*;
-- TestUserSecretsLimitsSchemaConstants asserts they match.
...
BEGIN
    ...
    -- Sum existing rows excluding the row being updated (so UPDATEs
