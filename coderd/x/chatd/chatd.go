...
func (p *Server) resolveAdvisorModelOverride(
	...
	// GetEnabledChatModelConfigByID joins on ai_providers.enabled = TRUE
	...
	// whose provider was just disabled, and a fallback key would let
	// ModelFromConfig succeed, silently routing advisor prompts to a provider
	// the admin expects to be off.
...
func isProviderAPIKeysEmpty(keys chatprovider.ProviderAPIKeys) bool {
	return keys.OpenAI == "" &&
		keys.Anthropic == "" &&
		len(keys.ByProvider) == 0 &&
		len(keys.BaseURLByProvider) == 0
}
...
func (p *Server) resolveModelConfigProviderHintAndKeys(
	...
	if !modelConfig.AIProviderID.Valid {
		if !isProviderAPIKeysEmpty(fallbackKeys) && userCanUseProviderKeys(fallbackKeys, providerHint) {
